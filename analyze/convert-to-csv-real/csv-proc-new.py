...
def process_proc_new_splitter(input_file_path):
    """
    Reads JSON new process data and splits it into multiple CSV files 
    based on the DevID structure.
    
    Path Structure: ./[suffix_path]/[prefix]-data/proc-new.csv
    """
    
    # Track initialized files to avoid redundant disk checks
    initialized_paths = set()

    # Fixed header for New Process data
    # (Note: Does not include 'rss_shmem' based on your provided script)
    header = [
    ...
    try:
        with open(input_file_path, 'r') as infile:
            for line in infile:
                ...
                output_file = output_dir / "proc-new.csv"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Splits New Process JSON logs into folders based on DevID.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument(
        "input_file",
        type=Path,
        help="Path to the input file with new process data."
    )

    args = parser.parse_args()
    process_proc_new_splitter(args.input_file)