...
def process_proc_cpu_splitter(input_file_path):
    """
    Reads JSON process/CPU data and splits it into multiple CSV files 
    based on the DevID structure.
    
    Path Structure: ./[suffix_path]/[prefix]-data/proc-cpu.csv
    """
    
    # Track initialized files to avoid redundant disk checks
    initialized_paths = set()

    # Define the fixed header structure for Process data
    header = [
    ...
    try:
        with open(input_file_path, 'r') as infile:
            for line in infile:
                ...
                output_file = output_dir / "proc-cpu.csv"
                ...
                row = [
                    ...
                    dev_id,                      # explicitly use the parsed dev_id
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Splits Process/CPU JSON logs into folders based on DevID.",
    ...
    process_proc_cpu_splitter(args.input_file)